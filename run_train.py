...
Options:
  ...
  --gpu=<id>      Comma separated GPU list. [default: 6,7]
...
from misc.utils import rm_n_mkdir, check_manual_seed
...
class TrainManager(Config):
    ...
    ####
    def _get_datagen(self, batch_size, run_mode, nr_procs=0, fold_idx=0):
            ...
            if pretrained_path is not None:
                if pretrained_path == -1:
                    ...
                    if chkpt_ext == "npz":
                        ...
                        net_state_dict = torch.load(pretrained_path)["desc"]
                    elif chkpt_ext == "pth":  # ! assume same saving format we desire
                        # weight_path = 'efficientnet_pytorch/efficientnetb0.pth'
                        net_state_dict = torch.load(pretrained_path)
                        net_state_dict = convert_encoder_checkpoint(net_state_dict)
                        net_state_dict = {k: v for k, v in net_state_dict.items()
                                             if net_desc.state_dict()[k].numel() == v.numel()
                                             }
                ...
                # if net_desc.freeze:
                #     for name, para in net_desc.named_parameters():
    ...
    def run(self):
        ...
        for phase_idx, phase_info in enumerate(phase_list):
            if len(phase_list) == 1:
                ...
                save_path = self.log_dir + f"/{self.dataset_name}_{self.seed}/{self.dataset_name}_seed_rgb/%02d/" % (phase_idx)
            # if phase_idx == 1:
            self.run_once(
                phase_info, engine_opt, save_path, prev_log_dir=prev_save_path
            )
            prev_save_path = save_path
            # prev_save_path = self.log_dir + f"/{self.dataset_name}_{self.seed}/{self.dataset_name}_seed/00"


####
if __name__ == "__main__":
    args = docopt(__doc__, version="Son-Net v1.0")
